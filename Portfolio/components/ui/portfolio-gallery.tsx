...
export function PortfolioGallery({
  ...
  const handlePrevious = () => {
    if (onProjectOpen && currentIndex > 0) {
      onProjectOpen(currentIndex - 1)
    }
  }

  const handleNext = () => {
    if (onProjectOpen && currentIndex < images.length - 1) {
      onProjectOpen(currentIndex + 1)
    }
  }
  ...
  return (
    <section className={cn("relative min-h-screen w-full overflow-hidden", className)}>
      ...
      <div
        className="absolute left-0 top-0 bottom-0 w-32 z-50 cursor-pointer"
      ...
      <div
        className="absolute right-0 top-0 bottom-0 w-32 z-50 cursor-pointer"
        onClick={handleNext}
        style={{
      ...
      <div className="hidden md:block relative overflow-hidden h-screen">
        <div className={cn("flex", spacing, "pb-8 pt-40 items-end justify-center h-full")}>
